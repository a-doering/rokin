...
class MovingArm(Robot):
    ...
    def get_frames_jac(self, q):
        ...
        n_dof_copy, self.n_dof = self.n_dof, self.n_dof_arm
        d = forward.create_frames_dict(f=f, robot=self)
        forward.combine_frames_jac(j=j[..., self.n_dim:], d=d, robot=self)
        self.n_dof = n_dof_copy
...
class MovingArm03(MovingArm):
    def __init__(self):
        super().__init__(n_dof=3)
        ...
        self.spheres_f_idx = np.array([0, 0])
    ...
    def get_frames_jac(self, q):
        ...
        fill_frames_2d_sc(f=j[..., 2], sin=cos, cos=-sin)
        _fill_frames_jac__dx(j=j, n_dim=self.n_dim)
...
class Blob03(MovingArm03):
    def __init__(self, n, r0=0.1, r1=0.4, seed=None):
        ...
        self.spheres_pos = v
        self.spheres_rad = r
        self.spheres_f_idx = np.zeros(n, dtype=int)
